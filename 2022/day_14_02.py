...
# if sand grain goes down to (void level - 1), make it a rock
# pour sand until starting sand position is a rock
#
# P.S. This came out super slow... I think it was running for like 20 minutes.
# Oh well, a star is a star!!
...
void_y_coord = max([c[1] for c in rock_coords]) + 2
...
# pour that sand
while True:
  if tmp_coord[0] == sand_entry_coord[0] and tmp_coord[1] == sand_entry_coord[1]:
    break
  ...
  while True:
    ...
    if tmp_coord_under not in rock_coords:
      ...
      # get out if we've reached the bottom
      if tmp_coord[1] == void_y_coord - 1:
      ...
      rock_coords.append(tmp_coord)
      print(str(sand_grains_count) + " " + str(tmp_coord[1]))
      sand_grains_count+=1
      break

print(str(sand_grains_count) + " units of sand come to rest.")
